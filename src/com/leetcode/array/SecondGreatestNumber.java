...
public class SecondGreatestNumber {
  ...
  public static void main(String[] args) {
    ...
    Arrays.stream(arr).boxed().max(Comparator.reverseOrder()).stream()
        .skip(1)
        .findFirst()
        .orElse(-1);
    ...
    var largestNumber = numList.stream().max(Integer::compareTo).orElse(-1);
    System.out.println("largestNumber:: "+largestNumber);

    var y =
        numList.stream().sorted(Comparator.reverseOrder()).skip(1).findFirst().orElseGet(() -> 1);
