...
public class ComparatorClass {
  ...
  public static void main(String[] args) {
    ...
    var kList =
        hackerList.stream().filter(num -> num == 2 || num == 4).collect(Collectors.toList());
    ...
    int finalsum =
        integerList.stream().map(e -> e * 2).filter(predicate).reduce(0, (ans, i) -> ans + i);
    ...
    var x = treeSet.stream().peek(peekResult::add).sorted().findFirst();
